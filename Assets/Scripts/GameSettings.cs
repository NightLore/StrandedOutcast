...
public class GameSettings : MonoBehaviour
{
    ...
    {
/* 0 */ new Weapon(     "Stick", recipe.Reset().Set(STICK, 1).GetRecipe(),                   STICK,  1, new Vector3(1.0f, 1.5f, 1.0f), 4.0f, int.MaxValue),
/* 1 */ new Weapon(      "Rock", recipe.Reset().Set(ROCK, 1) .GetRecipe(),                    ROCK,  4, new Vector3(1.0f, 1.0f, 1.0f), 1.0f, int.MaxValue),
/* 2 */ new Weapon(     "Metal", recipe.Reset().Set(METAL, 1).GetRecipe(),                   METAL,  2, new Vector3(1.0f, 1.0f, 1.0f), 2.0f, int.MaxValue),
/* 3 */ new Weapon(  "StoneAxe", recipe.Reset().Set(STICK, 2).Set(ROCK, 2).GetRecipe(),   STONEAXE,  6, new Vector3(4.0f, 1.0f, 4.0f), 2.0f, 15),
/* 4 */ new Weapon("StoneSpear", recipe.Reset().Set(STICK, 3).Set(ROCK, 1).GetRecipe(), STONESPEAR,  6, new Vector3(1.0f, 5.0f, 5.0f), 3.0f, 20),
/* 5 */ new Weapon("StoneKnife", recipe.Reset().Set(STICK, 1).Set(ROCK, 2).GetRecipe(), STONEKNIFE,  3, new Vector3(1.5f, 1.5f, 1.5f), 5.0f, 15),
/* 6 */ new Weapon( "StonePick", recipe.Reset().Set(STICK, 3).Set(ROCK, 3).GetRecipe(),  STONEPICK,  2, new Vector3(2.0f, 1.5f, 1.5f), 2.5f, 20),
/* 7 */ new Weapon( "BattleAxe", recipe.Reset().Set(STICK, 5).Set(METAL, 5).GetRecipe(), BATTLEAXE, 12, new Vector3(7.0f, 4.0f, 1.5f), 1.5f, 30),
/* 8 */ new Weapon(     "Sword", recipe.Reset().Set(STICK, 4).Set(METAL, 4).GetRecipe(),     SWORD,  8, new Vector3(3.0f, 3.0f, 1.5f), 4.0f, 30),
...
/*12 */ new Item(       "Forge", recipe.Reset().Set(STICK, 4).Set(ROCK, 8).GetRecipe()),
